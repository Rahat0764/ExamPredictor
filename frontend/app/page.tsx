  ...
  return (
    <div className="flex flex-col gap-12">
      <section className="text-center py-8">
        ...
        {/* Stats — সব animated */}
        <div className="flex justify-center gap-8 sm:gap-14 mb-10 flex-wrap">
          {[
            { value: stats.subjects, suffix: "+", label: "Subjects" },
            { value: stats.questions, suffix: "+", label: "Questions Analyzed" },
        ...
        <div className="flex gap-3 justify-center flex-wrap">
          <Link href="/predict" className="btn-primary-glow px-8 py-3.5 text-sm no-underline">🔮 Try Predictions</Link>
          <Link href="/upload/questions" className="btn-ghost-muted px-8 py-3.5 text-sm no-underline inline-flex items-center gap-2">📤 Upload Papers</Link>
          <Link href="/subjects" className="btn-ghost-muted px-8 py-3.5 text-sm no-underline inline-flex items-center gap-2">📊 Browse Subjects</Link>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <Link key={card.title} href={card.href} className="glass-card group relative overflow-hidden text-left no-underline block">
            ...
            <span style={{ position: "absolute", top: 20, right: 20, color: "var(--text-muted)", fontSize: 18 }} className="group-hover:translate-x-1 group-hover:-translate-y-1 group-hover:text-[var(--violet-light)] transition-transform">↗</span>
          </Link>
        ))}
      </section>
      ...
      <section className="glass-card" style={{ padding: "28px" }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            { step: "01", icon: "📥", title: "Upload Papers", desc: "Drag & drop past exam papers by year. Files stored in Google Drive." },
